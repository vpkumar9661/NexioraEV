...
import { ChevronRight, Home, LayoutDashboard, Compass, Layers, Landmark, Activity, HardDrive, FileText, BookOpen, Bot, Award, Play, X, RefreshCcw, Sun, Flame, Clock, Radio, ShieldAlert } from "lucide-react";
...
import { MicrogridDashboard } from "./components/MicrogridDashboard";
import { VirtualPowerPlant } from "./components/VirtualPowerPlant";
import { LoadBalancer } from "./components/LoadBalancer";
import { DemandForecast } from "./components/DemandForecast";
import { EnergyStorageCenter } from "./components/EnergyStorageCenter";
...
const SECTIONS = [
  ...
  { id: "grid-overview", label: "Grid Overview", icon: Radio },
  { id: "energy-flow", label: "Energy Flow", icon: Layers },
  { id: "microgrid", label: "Microgrid Management", icon: ShieldAlert },
  { id: "vpp", label: "Virtual Power Plant", icon: Landmark },
  { id: "load-balancer", label: "Load Balancer", icon: Activity },
  { id: "demand-forecast", label: "Demand Forecast", icon: Clock },
  { id: "battery-storage", label: "Battery Storage", icon: HardDrive },
  { id: "power-quality", label: "Power Quality", icon: Activity },
  { id: "energy-trading", label: "Energy Trading", icon: Landmark },
  { id: "carbon-analytics", label: "Carbon Analytics", icon: Activity },
  { id: "ai-consultant", label: "AI Energy Assistant", icon: Bot },
  { id: "report-center", label: "Reports", icon: FileText },
  { id: "knowledge-center", label: "Knowledge Center", icon: BookOpen },
];

export default function SmartEnergySolutionsPage() {
  ...
  const handleOptimizeGrid = () => {
    ...
    scrollToSection("load-balancer");
    alert("System Config: Edge load balancing parameters recalculated. Microgrid power distribution optimized.");
  };
    ...
    <div key={optimizerKey} className="h-[calc(100vh-80px)] mt-20 overflow-hidden bg-[#05070d] text-white relative font-sans">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 h-full flex flex-col pb-6">
        ...
        <nav className="flex items-center gap-2 text-[12px] font-bold text-muted-foreground/50 uppercase tracking-widest mb-4 shrink-0">
          ...
          <ChevronRight className="w-3 h-3" />
          <span className="text-[#00E676]">Smart Energy Solutions</span>
        </nav>
        ...
        <div className="flex gap-8 relative items-start flex-1 min-h-0">
          ...
          >
            ...
            <div id="hero">
              <EnergyHero
                onLaunchCenter={handleOptimizeGrid}
            ...
            {/* Section 3: Smart Grid Overview */}
            <div id="grid-overview">
              <GridOverview />
            </div>

            {/* Section 4: Real-Time Energy Flow */}
            <div id="energy-flow">
              <EnergyFlowCenter />
            </div>

            {/* Section 5: Microgrid Management */}
            <div id="microgrid">
              <MicrogridDashboard />
            </div>

            {/* Section 6: Virtual Power Plant */}
            <div id="vpp">
              <VirtualPowerPlant />
            </div>
            ...
            {/* Section 8: Demand Forecast */}
            <div id="demand-forecast">
              <DemandForecast />
            </div>

            {/* Section 9: Energy Storage */}
            <div id="battery-storage">
              <EnergyStorageCenter />
            </div>

            {/* Section 10: Power Quality */}
            <div id="power-quality">
              <PowerQuality />
            </div>

            {/* Section 11: Energy Trading */}
            <div id="energy-trading">
              <EnergyTrading />
            </div>

            {/* Section 12: Carbon Analytics */}
            <div id="carbon-analytics">
              <CarbonIntelligence />
            </div>

            {/* Section 13: AI Energy Assistant */}
            <div id="ai-consultant">
              <AIEnergyAssistant />
            </div>

            {/* Section 14: Report Center */}
            <div id="report-center">
              <ReportCenter />
            </div>

            {/* Section 15: Knowledge Center */}
        ...
        {showOptimizeToast && (
          ...
          >
            ...
            <div className="text-xs">
              <span className="font-extrabold text-white block">Compiling VPP Aggregations</span>
              <span className="text-[10px] text-muted-foreground/60 block mt-0.5">Sizing grid reserve assets and balancing node loads...</span>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
