...
import { ChevronRight, Home, Activity, Brain } from "lucide-react";
...
interface IntelligenceDashboardTemplateProps {
  title: string;
  subtitle: string;
  categoryName: string;
  stats: StatItem[];
...
export function IntelligenceDashboardTemplate({
  title,
  subtitle,
  categoryName,
  stats,
  widgets
}: IntelligenceDashboardTemplateProps) {
  return (
    <div className="min-h-screen bg-[#07090e] text-white pt-24 pb-16 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <nav className="flex items-center gap-2 text-[12px] font-medium text-muted-foreground/60">
          ...
          <ChevronRight className="w-3 h-3" />
          <span className="text-[#FF8C00] font-semibold">{categoryName}</span>
        </nav>

        {/* Glassmorphic Page Header */}
        <header className="relative overflow-hidden rounded-[20px] border border-[#FF8C00]/15 bg-linear-to-b from-[#FF8C00]/5 to-transparent p-6 sm:p-8 backdrop-blur-md shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)]">
          <div className="absolute top-0 right-0 p-4 opacity-10 pointer-events-none">
            <Brain className="w-40 h-40 text-[#FF8C00]" />
          </div>
          ...
          <div className="max-w-3xl space-y-3 relative z-10">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#FF8C00]/10 border border-[#FF8C00]/20 text-[#FF8C00] text-[11px] font-bold uppercase tracking-wider">
              NexioraEV Neural Node — AI Diagnostics Active
            </div>
            ...
            >
              <div className="px-5 py-4 border-b border-white/5 bg-white/2 flex items-center justify-between">
                ...
                <div className="flex gap-1">
                  <div className="w-2.5 h-2.5 rounded-full bg-[#FF8C00] animate-pulse" />
                </div>
              </div>
