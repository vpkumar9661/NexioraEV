...
interface EvtechDashboardTemplateProps {
  title: string;
  subtitle: string;
  categoryName: string;
  stats: StatItem[];
...
export function EvtechDashboardTemplate({
  title,
  subtitle,
  categoryName,
  stats,
  widgets
}: EvtechDashboardTemplateProps) {
  return (
    <div className="min-h-screen bg-[#07090e] text-white pt-24 pb-16 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <nav className="flex items-center gap-2 text-[12px] font-medium text-muted-foreground/60">
          ...
          <ChevronRight className="w-3 h-3" />
          <span className="text-[#FF9F1A]">{categoryName}</span>
        </nav>
        ...
        <header className="relative overflow-hidden rounded-[20px] border border-white/5 bg-linear-to-b from-white/4 to-transparent p-6 sm:p-8 backdrop-blur-md shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)]">
          <div className="absolute top-0 right-0 p-4 opacity-10 pointer-events-none">
            <Activity className="w-40 h-40 text-[#FF9F1A]" />
          </div>
          
          <div className="max-w-3xl space-y-3 relative z-10">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#FF9F1A]/10 border border-[#FF9F1A]/20 text-[#FF9F1A] text-[11px] font-bold uppercase tracking-wider">
              EVTech Node — Live Diagnostics
            </div>
            ...
            >
              <div className="px-5 py-4 border-b border-white/5 bg-white/2 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Activity className="w-4 h-4 text-[#FF9F1A]" />
