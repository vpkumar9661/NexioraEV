...
import { ChevronRight, Home, Activity, Landmark } from "lucide-react";
...
interface SchemesDashboardTemplateProps {
  title: string;
  subtitle: string;
  categoryName: string;
  stats: StatItem[];
...
export function SchemesDashboardTemplate({
  title,
  subtitle,
  categoryName,
  stats,
  widgets
}: SchemesDashboardTemplateProps) {
  return (
    <div className="min-h-screen bg-[#07090e] text-white pt-24 pb-16 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <nav className="flex items-center gap-2 text-[12px] font-medium text-[#AEB5C0]/60">
          ...
          <ChevronRight className="w-3 h-3" />
          <span className="text-[#4F46E5] font-semibold">{categoryName}</span>
        </nav>

        {/* Glassmorphic Page Header */}
        <header className="relative overflow-hidden rounded-[20px] border border-[#4F46E5]/15 bg-linear-to-b from-[#4F46E5]/5 to-transparent p-6 sm:p-8 backdrop-blur-md shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)]">
          <div className="absolute top-0 right-0 p-4 opacity-10 pointer-events-none">
            <Landmark className="w-40 h-40 text-[#4F46E5]" />
          </div>
          ...
          <div className="max-w-3xl space-y-3 relative z-10">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#4F46E5]/10 border border-[#4F46E5]/20 text-[#4F46E5] text-[11px] font-bold uppercase tracking-wider">
              NexioraEV Policy Node — Government Benefits Portal Active
            </div>
