    ...
    <section id="faq" className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground/60 mt-1">Common answers about high-voltage battery technologies</p>
        </div>
        ...
        <div className="relative w-full sm:w-[280px]">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2.5 rounded-xl bg-white/3 border border-white/10 text-sm text-white placeholder:text-muted-foreground/30 focus:outline-none focus:border-[#10B981]/40 focus:bg-[#10B981]/3 transition-all"
          />
        </div>
      </div>
        ...
        {filtered.map((faq, idx) => {
          ...
          return (
            ...
            >
              ...
              >
                <HelpCircle className={`w-4 h-4 shrink-0 transition-colors ${isOpen ? "text-[#6EE7B7]" : "text-muted-foreground/40"}`} />
                <span className={`flex-1 transition-colors ${isOpen ? "text-white" : "text-muted-foreground/85"}`}>
                  {faq.q}
                </span>
                <ChevronDown className={`w-4 h-4 text-muted-foreground/40 transition-transform duration-300 shrink-0 ${isOpen ? "rotate-180 text-[#6EE7B7]" : ""}`} />
              </button>
              <div className={`overflow-hidden transition-all duration-300 ${isOpen ? "max-h-[300px] opacity-100" : "max-h-0 opacity-0"}`}>
                <p className="px-5 pb-5 text-[12.5px] text-muted-foreground/70 leading-relaxed pl-12">
                  {faq.a}
                </p>
              </div>
            </div>
